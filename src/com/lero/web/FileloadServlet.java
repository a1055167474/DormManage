...
public class FileloadServlet extends HttpServlet{
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			request.setAttribute("mainPage", "fileload.jsp");
	...
	private void dormBuildDelete(HttpServletRequest request,
		...
		try {
			...
			request.getRequestDispatcher("fileload?action=list").forward(request, response);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			try {
	...
	private void dormBuildPreSave(HttpServletRequest request,
		...
		request.setAttribute("mainPage", "fileloadlist.jsp");
		request.getRequestDispatcher("mainAdmin.jsp").forward(request, response);
	}
	...
	private String genPagation(int totalNum, int currentPage, int pageSize){
		...
		pageCode.append("<li><a href=''>首页</a></li>");
		if(currentPage==1) {
		...
		pageCode.append("<li><a href=''>尾页</a></li>");
